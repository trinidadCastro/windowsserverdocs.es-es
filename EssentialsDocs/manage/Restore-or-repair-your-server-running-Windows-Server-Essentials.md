...
>Se aplica a: Windows Server 2016 Essentials, Windows Server 2012 R2 Essentials, Windows Server 2012 Essentials
 ...
 En este tema proporciona una visión general y procedimientos para restaurar o reparar un servidor que ejecuta Windows Server Essentials e incluye las secciones siguientes:  
  
-   [Información general de restauraciones de sistema de servidor](Restore-or-repair-your-server-running-Windows-Server-Essentials.md#BKMK_Overview)  
...
##  <a name="BKMK_Overview"></a> Información general de restauraciones de sistema de servidor  
...
###  <a name="BKMK_RestoreFromBackup"></a> Restaurar el servidor desde una copia de seguridad  
 ...
 Si está disponible una copia de seguridad, la mejor opción para restaurar el servidor es usar los medios de instalación de s de fabricante para restaurar desde una copia de seguridad externa. La restauración recuperará las carpetas y la configuración del servidor de la copia de seguridad que elija. Solo tiene que configurar las opciones y restaurar los datos que se crean después de la copia de seguridad.  
...
-   Si mantiene todos los datos, los datos del volumen del sistema se eliminan, pero se conservan los datos en otros volúmenes.  
    ...
    > [!CAUTION]
    >  Si la configuración de disco no coincide con la configuración predeterminada, se eliminarán todos los datos en un disco. Si reemplaza el disco del sistema, el nuevo disco debe ser mayor que el volumen del sistema de s disco original.  
    >   
...
-   **Volver a configurar el servidor.** En el servidor, use el asistente de configuración del servidor para volver a especificar las opciones de configuración. Para configurar un servidor administrado de forma remota de Windows Server Essentials desde un equipo cliente, abra un explorador web y, a continuación, escriba **http://***<YourServerName\>*  en la barra de direcciones.  
  
-   **Volver a conectar equipos cliente al servidor.** Si se conectó al servidor un equipo, debe desinstalar el software del conector de Windows Server Essentials desde el equipo antes de conectar el equipo al servidor nuevo. Para obtener más información, consulte [Uninstall the Connector software](../use/Get-Connected-in-Windows-Server-Essentials.md#BKMK_13) y [Connect computers to the server](../use/Get-Connected-in-Windows-Server-Essentials.md#BKMK_9).  
...
###  <a name="BKMK_Restore_1"></a> Restaurar o reparar el servidor usando medios de instalación  
 El siguiente procedimiento describe cómo restaurar la unidad del sistema de servidor desde una copia de seguridad mediante el uso de los medios de instalación de Windows Server Essentials. Para obtener información sobre cómo usar los medios de instalación para restaurar la configuración predeterminada de fábrica, consulte la documentación del fabricante del servidor.  
  
> [!NOTE]
>  Si el servidor usa espacios de almacenamiento y a restaurar los datos a un nuevo servidor, debe recuperar primero, la unidad del sistema y, a continuación, inicie sesión en el panel de Windows Server Essentials, configurar espacios de almacenamiento de una manera similar, como en el servidor anterior y, a continuación, recuperar la dat un volúmenes.  
...
5.  En el **recuperación de imagen del sistema** , seleccione el sistema actual? cualquier **Windows Server Essentials** o **Windows Server Essentials**.  
...
> [!NOTE]
>  Para restaurar y compartir carpetas en el servidor, debe realizar pasos adicionales. Para obtener más información, consulte [Restore files and folders on the server](Restore-or-repair-your-server-running-Windows-Server-Essentials.md#BKMK_RestoreFilesAndFolders).  
  
###  <a name="BKMK_Restore_2"></a> Restaurar o restablecer el servidor desde un equipo cliente mediante el DVD de recuperación  
 En Windows Server Essentials, puede iniciar el servidor desde una unidad flash USB de arranque que cree y, a continuación, recuperar el servidor desde un equipo cliente mediante el DVD que recibió del fabricante del servidor de recuperación. El equipo cliente debe estar en la misma red que el servidor. Este método no está disponible en Windows Server Essentials.  
...
1.  Abra el panel de Windows Server Essentials y, a continuación, haga clic en el **dispositivos** ficha.  
...
> [!WARNING]
>  Para obtener más información acerca de la copia de seguridad y restaurar archivos y carpetas, consulte [administrar copias de seguridad y restauración](Manage-Backup-and-Restore-in-Windows-Server-Essentials.md).  
  
###  <a name="BKMK_ConfigreSharedFolders"></a> Restaurar las carpetas compartidas en el servidor  
 Después de restaurar la unidad del sistema de servidor s, si las carpetas compartidas siguen en la partición de datos o se restauraron en la partición de datos, es posible que deba configurar las carpetas compartidas de nuevo para el servidor para que reconozca las carpetas. El siguiente procedimiento describe cómo agregar carpetas compartidas que se han compartido antes.  
...
3.  Inicie sesión en el panel de Windows Server Essentials, haga clic en el **almacenamiento** pestaña y, a continuación, haga clic en **agregar una carpeta** en el **tareas de carpetas de servidor** panel.  
  
     Se abrirá el asistente para agregar carpetas.  
  
4.  Escriba un nombre para el recurso compartido en el cuadro **Nombre**.  
  
5.  Haga clic en **examinar**, vaya a *< unidad\>\\< ServerName\>* \ServerFolders (por ejemplo *d:\Contoso\ServerFolders*), seleccione la carpeta que desea compartir y, a continuación, haga clic en **Aceptar**.  
...
7.  Especifique los permisos que anotó en el paso 2 y, a continuación, haga clic en **Agregar una carpeta**.  
    ...
    > [!IMPORTANT]
    >  Estos permisos reemplazarán los permisos existentes que no se agregaron a la carpeta utilizando el panel de Windows Server Essentials.  
