...
|Tarea|Descripción|  
|----|--------|  
|Planear la topología de red y la configuración|Decide dónde colocar el servidor de DirectAccess @no__t 0at el borde o detrás de una traducción de direcciones de red \(NAT @ no__t-2, el dispositivo o el Firewall @ no__t-3, y planea el direccionamiento IP y el enrutamiento.|  
...
## <a name="bkmk_1_1_Network_svr_top_settings"></a>Planear la topología de red y la configuración  
...
2.  Identifica tus requisitos de direccionamiento IP:  
    ...
    DirectAccess usa IPv6 con IPsec para crear una conexión segura entre los equipos cliente de DirectAccess y la red corporativa interna. Sin embargo, DirectAccess no requiere necesariamente conectividad con Internet IPv6 ni compatibilidad nativa con IPv6 en las redes internas. En su lugar, configura y usa automáticamente tecnologías de transición IPv6 para Tunelizar el tráfico IPv6 a través de Internet IPv4 \(6to4, Teredo, IP @ no__t-1HTTPS @ no__t-2 y a través de la intranet @ no__t-3only intranet \(NAT64 o ISATAP @ no__t-5. Para obtener información general acerca de estas tecnologías de transición, consulta los siguientes recursos:  
...
### <a name="ConfigFirewalls"></a>Planear los requisitos del firewall  
...
-   IP @ no__t-0HTTPS-protocolo de control de transmisión \(TCP @ no__t-2 puerto de destino 443 y puerto de origen TCP 443 de salida.  
...
-   TCP @ no__t-0UDP para todo el tráfico IPv4 @ no__t-1IPv6  
...
1.  **IP @ no__t-1HTTPS Server**. Al configurar DirectAccess, el servidor de DirectAccess se configura automáticamente para que actúe como el agente de escucha Web de IP @ no__t-0HTTPS. El sitio IP @ no__t-0HTTPS requiere un certificado de sitio web y los equipos cliente deben poder ponerse en contacto con el sitio de la lista de revocación de certificados \(CRL @ no__t-2 para el certificado. El Asistente para habilitar DirectAccess intenta usar el certificado SSTP de VPN. Si SSTP no está configurado, comprueba si hay un certificado para IP @ no__t-0HTTPS en el almacén personal del equipo. Si no hay ninguno disponible, se crea automáticamente un certificado Self @ no__t-0signed.
  
2.  **Servidor de ubicación de red**. El servidor de ubicación de red es un sitio web que se usa para detectar si los equipos cliente se encuentran en la red corporativa. El servidor de ubicación de red requiere un certificado de sitio Web. Los clientes de DirectAccess tienen que poder contactar con el sitio de la CRL para obtener el certificado. El Asistente para habilitar acceso remoto comprueba si existe un certificado para el servidor de ubicación de red en el almacén personal del equipo. Si no está presente, crea automáticamente un certificado Self @ no__t-0signed.
  
En la tabla siguiente encontrarás un resumen de los requisitos de certificación para cada uno de estos:  
  
|Autenticación IPsec|IP @ no__t-0HTTPS Server|Servidor de ubicación de red|  
|------------|----------|--------------|  
|Se requiere una entidad de certificación interna para emitir certificados de equipo al servidor y a los clientes de DirectAccess para la autenticación IPsec cuando no se usa el proxy Kerberos para la autenticación.|CA pública: se recomienda usar una entidad de certificación pública para emitir el certificado IP @ no__t-0HTTPS, lo que garantiza que el punto de distribución de CRL esté disponible externamente.|CA interna: puede usar una CA interna para emitir el certificado de sitio web del servidor de ubicación de red. Asegúrate de que el punto de distribución de CRL tenga alta disponibilidad desde la red interna.|  
||CA interna: puede usar una CA interna para emitir el certificado IP @ no__t-0HTTPS; sin embargo, debe asegurarse de que el punto de distribución de CRL está disponible externamente.|Self @ no__t-0signed Certificate: puede usar un certificado Self @ no__t-1signed para el sitio web del servidor de ubicación de red. sin embargo, no puede usar un certificado Self @ no__t-2signed en implementaciones multisitio.|  
||Self @ no__t-0signed Certificate: puede usar un certificado Self @ no__t-1signed para el servidor IP @ no__t-2HTTPS. sin embargo, debe asegurarse de que el punto de distribución de CRL está disponible externamente. No se puede usar un certificado Self @ no__t-0signed en una implementación multisitio.||  
  
#### <a name="bkmk_website_cert_IPHTTPS"></a>Planear los certificados para IP @ no__t-1HTTPS y el servidor de ubicación de red  
Si quieres aprovisionar un certificado para estos objetivos, consulta [Implementar un único servidor de DirectAccess con configuración avanzada](../single-server-advanced/Deploy-a-Single-DirectAccess-Server-with-Advanced-Settings.md). Si no hay ningún certificado disponible, el Asistente para Introducción crea automáticamente certificados Self @ no__t-0signed para estos propósitos.
  
> [!NOTE]
> Si aprovisiona certificados para IP @ no__t-0HTTPS y el servidor de ubicación de red manualmente, asegúrese de que los certificados tengan un nombre de sujeto. Si el certificado no tiene un nombre de sujeto, sino que tiene un nombre alternativo, el Asistente para DirectAccess no lo aceptará.  
...
-   **Solicitudes de cliente de DirectAccess**. DNS se usa para resolver las solicitudes de equipos cliente de DirectAccess que no están ubicados en la red interna. Los clientes de DirectAccess intentan conectarse al servidor de ubicación de red de DirectAccess para determinar si están ubicados en Internet o en la red corporativa: Si la conexión es correcta, se determinará que los clientes están en la intranet, por lo que no se usará DirectAccess y las solicitudes de clientes se resolverán mediante el servidor DNS configurado en el adaptador de red del equipo cliente. Si se producen errores de conexión, se da por hecho que los clientes están en Internet. Los clientes de DirectAccess usarán la tabla de directivas de resolución de nombres \(NRPT @ no__t-1 para determinar qué servidor DNS se usará para resolver las solicitudes de nombres. Puedes especificar que los clientes tengan que usar DNS64 de DirectAccess para resolver nombres, o bien un servidor DNS interno alternativo. Para llevar a cabo la resolución de nombres, los clientes de DirectAccess usan la tabla NRPT para identificar cómo gestionar una solicitud. Los clientes solicitan un FQDN o un nombre de @ no__t-0label único como http: \/ @ no__t-2internal. Si un único nombre de @ no__t-0label es requests, se anexa un sufijo DNS para crear un FQDN. Si la consulta DNS coincide con una entrada de la NRPT y se especifica DNS4 o un servidor DNS de intranet para la entrada, la consulta para la resolución de nombres se enviará mediante el servidor especificado. Si existe una coincidencia pero no se especifica ningún servidor DNS, esto indica una regla de exención y se aplicará la resolución de nombres normal.  
    ...
    1.  Si la red corporativa es IPv4 @ no__t-0based, o IPv4 e IPv6, la dirección predeterminada es la dirección DNS64 del adaptador interno en el servidor de DirectAccess.  
  
    2.  Si la red corporativa es IPv6 @ no__t-0based, la dirección predeterminada es la dirección IPv6 de los servidores DNS de la red corporativa.  
...
-   **Servidores de infraestructura**
    ...
    1.  **Servidor de ubicación de red**. Los clientes de DirectAccess intentan contactar con el servidor de ubicación de red para determinar si están en la red interna. Los clientes de la red interna deben ser capaces de resolver el nombre del servidor de ubicación de red, pero debe evitarse que resuelvan el nombre si se encuentran en Internet. Para que esto suceda, de manera predeterminada, el FQDN del servidor de ubicación de red se agregará como una regla de exención a la NRPT. Además, al configurar DirectAccess, se crean automáticamente las reglas siguientes:  
        ...
        2.  Una regla de exención para el FQDN del servidor de ubicación de red. Por ejemplo, si la dirección URL del servidor de ubicación de red es https: @no__t -0\/nls.corp.contoso.com, se crea una regla de exención para el FQDN nls.corp.contoso.com.  
  
        **IP @ no__t-1HTTPS Server**. El servidor de DirectAccess actúa como agente de escucha de IP @ no__t-0HTTPS y usa su certificado de servidor para autenticarse en los clientes IP @ no__t-1HTTPS. Los clientes de DirectAccess que usan servidores DNS públicos deben poder resolver el nombre de IP @ no__t-0HTTPS.  
        ...
        **Comprobadores de la conectividad**. DirectAccess crea un sondeo web predeterminado que usan los equipos cliente de DirectAccess para comprobar la conectividad a la red interna. Para comprobar si el sondeo funciona correctamente es necesario registrar de forma manual los nombres siguientes en DNS:  
        ...
        1.  DirectAccess @ no__t-0webprobehost: debe resolverse en la dirección IPv4 interna del servidor de DirectAccess o en la dirección IPv6 en un entorno IPv6 @ no__t-1only.  
  
        2.  DirectAccess @ no__t-0corpconnectivityhost: debe resolverse en localhost \(loopback @ no__t-2 Address. Es necesario crear un registro A y un registro AAAA: el registro A con el valor 127.0.0.1 y el registro AAAA con el valor compuesto a partir del prefijo NAT64 con los últimos 32 bits como 127.0.0.1. El prefijo NAT64 se puede recuperar ejecutando el cmdlet Get @ no__t-0netnattransitionconfiguration.  
...
Si desea implementar un servidor de ubicación de red y no usar certificados Self @ no__t-0signed, consulte [implementación de un solo servidor de DirectAccess con configuración avanzada](../single-server-advanced/Deploy-a-Single-DirectAccess-Server-with-Advanced-Settings.md).
  
### <a name="bkmk_1_6_AD"></a>Active Directory del plan  
...
-   Al menos un controlador de dominio instalado en Windows Server 2016, Windows Server 2012 R2, Windows Server 2012, Windows Server 2008 R2 o Windows Server 2008. 
    ...
    Si el controlador de dominio está en una red perimetral \(and, por lo tanto, es accesible desde el adaptador de red de Internet @ no__t-1facing del servidor de DirectAccess @ no__t-2 para impedir que el servidor de DirectAccess lo alcance mediante la adición de filtros de paquetes en el dominio. controlador, para impedir la conectividad con la dirección IP del adaptador de Internet.  
...
-   Los clientes de DirectAccess deben ser miembros del dominio. Los clientes pueden pertenecer a:  
    ...
    -   Cualquier dominio que tenga una confianza @ no__t-0way con el dominio del servidor de DirectAccess.  
  
    -   Cualquier dominio de un bosque que tenga una confianza @ no__t-0way con el bosque al que pertenece el dominio de DirectAccess.  
  
> [!NOTE]
> - El servidor de DirectAccess no puede ser un controlador de dominio.  
> - El controlador de dominio de Active Directory que se usa para DirectAccess no debe ser accesible desde el adaptador de Internet externo del servidor de DirectAccess \(The el adaptador no debe estar en el perfil de dominio del firewall de Windows @ no__t-1.  
  
### <a name="bkmk_1_7_GPOs"></a>Planear objetos directiva de grupo  
La configuración de DirectAccess configurada al configurar DirectAccess se recopila en objetos de directiva de grupo \(GPO @ no__t-1. Se rellenan dos GPO distintos con opciones de DirectAccess, y se distribuyen así:  
...
> [!IMPORTANT]
> Independientemente de si usas GPO configurados automáticamente o manualmente, tendrás que agregar una directiva para la detección de vínculos de baja velocidad si tus clientes van a usar 3G. La ruta de acceso de directiva de grupo para **Policy: Configure directiva de grupo detección de vínculos de baja velocidad @ no__t-0 es: **Configuración del equipo \/ directivas \/ Plantillas administrativas \/ sistema \/ Directiva de grupo**.  
  
> [!CAUTION]  
> Haz lo siguiente para realizar una copia de seguridad de todos los objetos de directiva de grupo de DirectAccess antes de ejecutar los cmdlets de DirectAccess: [Copia de seguridad y restauración de la configuración de DirectAccess](https://go.microsoft.com/fwlink/?LinkID=257928)  
  
#### <a name="automatically-created-gpos"></a>GPO @ no__t-0created automáticamente  
Tenga en cuenta lo siguiente al usar los GPO @ no__t-0created automáticamente:  
...
#### <a name="manually-created-gpos"></a>GPO @ no__t-0created manualmente  
Tenga en cuenta lo siguiente al usar los GPO @ no__t-0created manualmente:  
  
-   Los GPO deben existir antes de ejecutar el Asistente para introducción de acceso remoto.  
  
-   Al usar los GPO @ no__t-0created manualmente, para aplicar la configuración de DirectAccess, el administrador de DirectAccess necesita permisos de GPO completos \(Edit, Delete, Modify Security @ no__t-2 en los GPO @ no__t-3created manualmente.  
...
Si un servidor de DirectAccess, un cliente o un GPO de servidor de aplicaciones se ha eliminado por accidente y no hay ninguna copia de seguridad disponible, debe quitar los valores de configuración y volver a @ no__t-0configure. Si hay una copia de seguridad disponible, puedes usarla para restaurar el GPO.  
  
Se mostrará el siguiente mensaje de error en **Administración de DirectAccess**: **No se encuentra el GPO <GPO name>** . Para quitar las opciones de configuración, sigue estos pasos:  
  
1.  Ejecute el cmdlet de PowerShell **Uninstall @ no__t-1remoteaccess**.  
  
2.  Re @ no__t-0open **Administración de DirectAccess**.  
  
3.  Verás un mensaje de error que indica que no se encontró el GPO. Haz clic en **Quitar opciones de configuración**. Una vez finalizado, el servidor se restaurará a un estado un @ no__t-0configured.  
