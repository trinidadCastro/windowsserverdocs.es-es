...
**Peticiones de credenciales y consentimiento**
...
|Comprobación del nivel de control deslizante de UAC|Ahora UAC ofrece la posibilidad de elegir entre cuatro niveles de notificación distintos usando un control deslizante:<br /><br /><ul><li>Alto<br /><br />    Cuando el control deslizante se establece en **Notificarme siempre**, el sistema comprueba si el escritorio seguro está habilitado</li><li>Medio<br /><br />    Si el control deslizante está establecido en **predeterminado-Notificarme solo cuando los programas intenten realizar cambios en el equipo**, el control de cuenta **User: Solo se comprueba la configuración de directiva elevar archivos ejecutables firmados y validados @ no__t-0:<br /><br /><ul><li>Si está habilitada, se aplica la validación de la ruta de certificación de la infraestructura de clave pública (PKI) a un archivo ejecutable concreto para que pueda ejecutarse.</li><li>Si no está habilitada (que es el valor predeterminado), no se aplica la validación de la ruta de certificación de la infraestructura de clave pública (PKI) a un archivo ejecutable concreto para que pueda ejecutarse. La configuración de directiva **Control de cuentas de usuario: Cambie al escritorio seguro cuando se pida confirmación de elevación @ no__t-0 directiva de grupo está activada la opción.</li></ul></li><li>Bajo<br /><br />    Cuando el control deslizante se establece en **Notificarme sólo cuando un programa intente realizar cambios en el equipo (no atenuar el escritorio)** , se llama a CreateProcess.</li><li>No notificarme nunca<br /><br />    Si el control deslizante se establece en no **Notificarme nunca cuando**, el símbolo del sistema de UAC nunca notificará cuando un programa esté intentando instalar o intentando realizar algún cambio en el equipo. **Importante:**     Esta configuración no es recomendable. Esta configuración es la misma que la configuración del control de cuenta **User: Comportamiento de la petición de elevación para los administradores en modo de aprobación de administrador @ no__t-0 configuración de directiva para **elevar sin preguntar**.</li></ul>|
|Escritorio seguro habilitado|La configuración de directiva **Control de cuentas de usuario: Cambiar al escritorio seguro cuando se pida confirmación de elevación @ no__t-0 está activada:<br /><br />-Si el escritorio seguro está habilitado, todas las solicitudes de elevación van al escritorio seguro, independientemente de la configuración de la Directiva de comportamiento de petición para administradores y usuarios estándar.<br />-Si el escritorio seguro no está habilitado, todas las solicitudes de elevación van al escritorio del usuario interactivo y se utiliza la configuración por usuario para los administradores y los usuarios estándar.|
...
> [!IMPORTANT]
> Para deshabilitar completamente UAC, debe deshabilitar el control de cuentas **User de la Directiva: Ejecutar todos los administradores en modo de aprobación de administrador @ no__t-0.
