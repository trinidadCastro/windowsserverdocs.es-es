  ...
  <section address="BKMK_Cause">
    ...
    <content>
      ...
      <list class="ordered">
        ...
        <listItem>
          <para>Máquina del servidor&#39;s EPM responde con un punto de conexión </para>
        </listItem>
        <listItem>
      ...
      <list class="ordered">
        <listItem>
          <para>La aplicación de servidor se ha iniciado nunca (es decir, paso 1 en el &quot;obtener más información&quot; nunca llegó a situada encima del diagrama).</para>
        </listItem>
        <listItem>
          <para>Inicia la aplicación de servidor, pero se produjo algún error durante la inicialización que impidió su registro con el Endpoint Mapper de RPC (es decir, paso 1 en el &quot;obtener más información&quot; diagrama anterior se intentó pero no se pudo).</para>
        </listItem>
        <listItem>
          <para>La aplicación de servidor se inició, pero posteriormente muerto. (es decir, paso 1 en el &quot;obtener más información&quot; diagrama anterior se completó correctamente, pero se deshizo más adelante, porque el servidor muerto).</para>
        </listItem>
        <listItem>
...
c:&gt;nslookup -type=cname &lt;fully qualified cname of source DC&gt; &lt;destination DCs secondary DNS Server IP&gt;</code>
          <para>Compruebe que la dirección IP devuelta por NSLOOKUP &quot;posee&quot; el nombre de host / identidad de seguridad de lo DC de origen:</para>
          <code>C:&gt;NBTSTAT -A &lt;IP address returned by NSLOOKUP in the step above&gt;</code>
          <para>o bien</para>
          <para>Inicie sesión en la consola del origen de controlador de dominio, ejecutar &quot;IPCONFIG&quot; desde el CMD símbolo del sistema y compruebe que el DC de origen pertenece a la dirección IP devuelta por el comando NSLOOKUP anterior</para>
...
<para>Si hay direcciones IP no válidas en los registros de host, investigue si está habilitada y configurada correctamente la limpieza de DNS. </para><para>Si las pruebas anteriores o en una red de seguimiento&#39;mostrar una consulta con nombre devuelve una dirección IP no válida, considere la posibilidad de las entradas obsoletas en los archivos del HOST, archivos LMHOSTS y servidores WINS. Tenga en cuenta que los servidores DNS también puede configurarse para realizar la resolución de reserva de WINS.</para>
</listItem>
        <listItem>
          <para>
...
&gt;netsh int ipv6 show dynamicport udp</code>
            ...
            <listItem>
              ...
              <para>Revisión <externalLink> <linkText>artículo de KB 224196</linkText> <linkUri> <a href="https://support.microsoft.com/kb/224196" data-raw-source="https://support.microsoft.com/kb/224196"> https://support.microsoft.com/kb/224196 </a> </linkUri> </externalLink> y asegúrese de que el puerto codificado se encuentra dentro del intervalo de puertos efímeros para el DC de origen&#39;s, versión del sistema operativo.</para>
            </listItem>
            <listItem>
...
ncacn_ip_udp REG_SZ rpcrt4.dll</code>
  ...
  <section address="BKMK_MoreInfo">
    ...
    <content>
      ...
      <para>El dominio contoso.com consta de DC1 y DC2 con x.x.1.1 de direcciones IP y x.x.1.2. El host &quot;A&quot; / &quot;AAAA&quot; registros para DC2 están registrados correctamente en todos los servidores DNS configurados para DC1. Además, el archivo de HOSTS en DC1 contiene una entrada de asignación de nombre de host DC2s completo a x.x.1.2 de dirección IP. Más adelante, DC2&#39;dirección IP cambia de X.X.1.2 a X.X.1.3 y un nuevo miembro está unido al dominio con x.x.1.2 de dirección IP. La replicación de AD intentos desencadenan por la <ui>Replicar ahora</ui> comando en el complemento Servicios y sitios de Active Directory produce error 1753 tal como se muestra en el siguiente mensaje de seguimiento:</para>
...
<codeFeaturedElement>11</codeFeaturedElement> x.x.1.2 x.x.1.1 EPM:Response: ept_map: 0x16C9A0D6 - EP_S_NOT_REGISTERED
      ...
      <para>Resumen: En este ejemplo no se pudo porque una asignación de IP de host no válido (en el archivo HOST en este caso) ocasionó que el destino de controlador de dominio para resolver un &quot;origen&quot; controlador de dominio que no tenía la ejecución del servicio de Active Directory Domain Services (o incluso instalado para esa cuestión) por lo que el SPN no se ha registrado la replicación y el DC de origen devuelven el error 1753. En el segundo caso, una asignación de IP de host no válido (nuevo en el archivo HOST) provocó el controlador de dominio para conectarse a un controlador de dominio que se había registrado el E351 destino... SPN de replicación, pero ese origen tenía una identidad diferente del nombre de host y la seguridad del controlador de dominio de origen deseada por lo que no se pudo los intentos con error-2146893022: El nombre principal de destino es incorrecto.</para>
    </content>
  </section>
  <relatedTopics>
    <externalLink>
...
<externalLink> <linkText>839880 errores Endpoint Mapper de RPC de solución de problemas con el soporte técnico de Windows Server 2003 de artículo de KB Las herramientas desde el CD del producto</linkText><linkUri><a href="https://support.microsoft.com/kb/839880" data-raw-source="https://support.microsoft.com/kb/839880">https://support.microsoft.com/kb/839880</a></linkUri></externalLink>
<externalLink><linkText>KB artículo 832017 información general y red puerto del servicio requisitos para el sistema de Windows Server</linkText><linkUri><a href="https://support.microsoft.com/kb/832017/" data-raw-source="https://support.microsoft.com/kb/832017/">https://support.microsoft.com/kb/832017/</a></linkUri></externalLink>
<externalLink><linkText>224196 restringir Active el artículo KB Tráfico de replicación de directorios y de cliente RPC de tráfico a un puerto específico</linkText><linkUri><a href="https://support.microsoft.com/kb/224196/" data-raw-source="https://support.microsoft.com/kb/224196/">https://support.microsoft.com/kb/224196/</a></linkUri></externalLink>
<externalLink><linkText>artículo de Knowledge Base 154596 cómo configurar la asignación de puertos dinámicos RPC para trabajar con firewalls</linkText><linkUri><a href="https://support.microsoft.com/kb/154596" data-raw-source="https://support.microsoft.com/kb/154596">https://support.microsoft.com/kb/154596</a></linkUri></externalLink><externalLink><linkText>cómo RPC Works</linkText><linkUri><a href="https://msdn.microsoft.com/library/aa373935(VS.85).aspx" data-raw-source="https://msdn.microsoft.com/library/aa373935(VS.85).aspx">https://msdn.microsoft.com/library/aa373935(VS.85).aspx</a></linkUri></externalLink><externalLink><linkText>cómo prepara el servidor para una conexión</linkText> <linkUri> <a href="https://msdn.microsoft.com/library/aa373938(VS.85).aspx" data-raw-source="https://msdn.microsoft.com/library/aa373938(VS.85).aspx"> https://msdn.microsoft.com/library/aa373938(VS.85).aspx </a> </linkUri> </externalLink> 
<externalLink> <linkText>Cómo el cliente establece una conexión</linkText> <linkUri> <a href="https://msdn.microsoft.com/library/aa373937(VS.85).aspx" data-raw-source="https://msdn.microsoft.com/library/aa373937(VS.85).aspx"> https://msdn.microsoft.com/library/aa373937(VS.85).aspx </a> </linkUri> </externalLink> <externalLink> <linkText>Registrar la interfaz</linkText> <linkUri> <a href="https://msdn.microsoft.com/library/aa375357(VS.85).aspx" data-raw-source="https://msdn.microsoft.com/library/aa375357(VS.85).aspx"> https://msdn.microsoft.com/library/aa375357(VS.85).aspx </a> </linkUri> </externalLink> <externalLink> <linkText>Hacer que el servidor estén disponibles en la red</linkText> <linkUri> <a href="https://msdn.microsoft.com/library/aa373974(VS.85).aspx" data-raw-source="https://msdn.microsoft.com/library/aa373974(VS.85).aspx"> https://msdn.microsoft.com/library/aa373974(VS.85).aspx </a> </linkUri> </externalLink> <externalLink> <linkText>Extremos registrados</linkText> <linkUri> <a href="https://msdn.microsoft.com/library/aa375255(VS.85).aspx" data-raw-source="https://msdn.microsoft.com/library/aa375255(VS.85).aspx"> https://msdn.microsoft.com/library/aa375255(VS.85).aspx </a> </linkUri> </externalLink> <externalLink> <linkText>Escuchar las llamadas al cliente</linkText> <linkUri> <a href="https://msdn.microsoft.com/library/aa373966(VS.85).aspx" data-raw-source="https://msdn.microsoft.com/library/aa373966(VS.85).aspx"> https://msdn.microsoft.com/library/aa373966(VS.85).aspx </a> </linkUri> </externalLink> <externalLink> <linkText>Cómo el cliente establece una conexión</linkText><linkUri><a href="https://msdn.microsoft.com/library/aa373937(VS.85).aspx" data-raw-source="https://msdn.microsoft.com/library/aa373937(VS.85).aspx">https://msdn.microsoft.com/library/aa373937(VS.85).aspx</a></linkUri></externalLink><span class=" class=""></span class="></linkText><linkUri><a href="https://msdn.microsoft.com/library/dd207688(PROT.13).aspx" data-raw-source="https://msdn.microsoft.com/library/dd207688(PROT.13).aspx">https://msdn.microsoft.com/library/dd207688(PROT.13).aspx</a></linkUri></externalLink></relatedTopics>
</developerConceptualDocument>
