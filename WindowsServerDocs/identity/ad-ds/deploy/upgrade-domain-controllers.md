...
|Si está ejecutando esta edición|Puede actualizar a estas ediciones|
| ----- | ----- |
...
1. Instalar los controladores de dominio que ejecutan Windows Server 2016.
...
1. En **administrador del servidor**, en **agregar Roles y características**, instalar **Active Directory Domain Services** en Windows Server 2016. Esto ejecutará automáticamente adprep en el dominio y bosque de R2 2012.
...
1. En el **configuración de implementación** pantalla, seleccione **agregar un controlador de dominio a un bosque existente** y haga clic en siguiente.

   ![Actualizar versión](media/Upgrade-Domain-Controllers-to-Windows-Server-2016/upgrade5.png)

1. En el **opciones del controlador de dominio** pantalla, escriba el **el modo de restauración de servicios de directorio (DSRM)** contraseña y haga clic en siguiente.
1. Para el resto de las pantallas, haga clic en **siguiente**.
1. En el **comprobación de requisitos previos** pantalla, haga clic en **instalar**. Una vez que ha completado el reinicio se puede volver inicio de sesión.
1. En el servidor de Windows Server 2012 R2, en **administrador del servidor**, en herramientas, seleccione **módulo Active Directory para Windows PowerShell**.

   ![Actualizar versión](media/Upgrade-Domain-Controllers-to-Windows-Server-2016/upgrade6.png)

1. En windows PowerShell use el ADDirectoryServerOperationMasterRole Move para mover los roles FSMO. Puede escribir el nombre de cada OperationMasterRole - o use números para especificar los roles. Para obtener más información, consulte [ADDirectoryServerOperationMasterRole de movimiento](https://technet.microsoft.com/library/hh852302.aspx)
...
1. Compruebe los roles se han movido para ello, vaya al servidor de Windows Server 2016, en **administrador del servidor**, en **herramientas**, seleccione **módulo Active Directory para Windows PowerShell**. Use la `Get-ADDomain` y `Get-ADForest` cmdlets para ver los titulares del rol FSMO.
...
1. Disminuir de nivel y quitar el controlador de dominio de Windows Server 2012 R2. Para obtener información acerca de degradar un controlador de dominio, consulte [degradación de controladores de dominio y dominios](../../ad-ds/deploy/Demoting-Domain-Controllers-and-Domains--Level-200-.md)
1. Una vez que se degradan y quitado el servidor puede generar el funcional del bosque y los niveles funcionales de dominio a Windows Server 2016.
