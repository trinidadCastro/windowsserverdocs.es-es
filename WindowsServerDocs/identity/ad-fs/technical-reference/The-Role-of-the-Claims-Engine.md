...
Como se muestra en la siguiente ilustración, la acción de aceptar las \(reglas\)de aceptación de notificaciones entrantes \(, autorizar\) a los solicitantes \( de notificaciones de autorización y emitir notificaciones salientes. el motor\) de notificaciones realiza las reglas de emisión a través de las reglas de notificación en todas las relaciones de confianza federadas de su organización.  
...
Cuando se configura una confianza de proveedor de notificaciones o una relación de confianza para usuario autenticado en su organización con\(reglas\) de notificación, el conjunto de reglas de notificación para esa relación de confianza actúa como un equipo selector para las notificaciones entrantes al invocar el motor de notificaciones para aplicar el lógica en las reglas de notificación para determinar si se emiten notificaciones y qué notificaciones se van a emitir.  
...
Si el conjunto de reglas de notificaciones que se ejecuta durante el paso 2 del proceso de ejecución de reglas de \(notificaciones consta de reglas de autorización que tienen conjuntos de notificaciones de entrada\)y salida diferentes que las reglas de aceptación o de emisión, entonces las reglas de autorización se ejecutarán para determinar si un solicitante de token está autorizado a obtener un token de seguridad para un usuario de confianza determinado desde el Servicio de federación en función de las notificaciones del solicitante.  
  
El objetivo de las reglas de autorización es emitir una notificación de denegación o de permiso, según si el usuario está habilitado o no para obtener un token para el usuario autenticado proporcionado. Tal y como se muestra en la siguiente ilustración, la canalización usa la salida de la ejecución de autorización para determinar si se ejecuta o no el conjunto de reglas de emisión, en función de la\/presencia o ausencia de la demanda de permiso o denegación, pero de la autorización. la salida de la ejecución no se usa como entrada para el conjunto de reglas de notificaciones.  
