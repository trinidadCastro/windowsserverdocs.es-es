...
- **EnableExtranetLockout &lt;boolean&gt;** establezca este valor booleano en true si desea habilitar el bloqueo de extranet.
...
- **ExtranetObservationWindow &lt;TimeSpan&gt;** determina durante cuánto tiempo se bloqueará la cuenta de usuario. AD FS comenzará a realizar la autenticación de nombre de usuario y contraseña cuando se pase la ventana. AD FS usa el atributo de AD badPasswordTime como referencia para determinar si la ventana de observación de extranet se ha superado o no. La ventana ha pasado si la hora actual > badPasswordTime + ExtranetObservationWindow. 
...
- **ExtranetLockoutRequirePDC &lt;&gt;booleano** : cuando está habilitada: el bloqueo de extranet requiere un controlador de dominio principal (PDC). Cuando se deshabilita: el bloqueo de la extranet se reservará a otro controlador de dominio en caso de que el PDC no esté disponible.
...
- **Restablecer contador de bloqueo de cuenta después**de: esta opción determina cuánto tiempo debe transcurrir desde el último error de inicio de sesión del usuario antes de que **badPwdCount** se restablezca en 0. Para que la característica de bloqueo de extranet en AD FS funcione correctamente con la Directiva de bloqueo de AD, debe asegurarse de que el valor de ExtranetObservationWindow en AD FS &gt; el contador de bloqueo de cuenta de restablecimiento después del valor de AD. En los ejemplos siguientes se explicará por qué.  
...
- Para evitar el bloqueo malintencionado de cuentas, debe asegurarse de que **ExtranetLockoutThreshold** < **umbral de bloqueo de cuenta** y **ExtranetObservationWindow** > restablecer el contador de **bloqueo de cuenta** .
