...
description: Controles operativos de seguridad para dominios de Active Directory de Windows Server
ms.topic: article
...
- El software procede de Internet y se valida mediante la descarga y comparación de dos copias independientes:
   ...
   - Compara los archivos descargados con una utilidad como certutil: `certutil -hashfile <filename>`
...
- Esta cuenta debe usarse solo para:
  ...
  - Realizar tareas poco frecuentes, como:
    ...
    - Tareas para todo el bosque que requieren privilegios administrativos de empresa
      ...
      > [!NOTE]
      > Ten en cuenta que se delega la administración de topologías, incluida la administración de sitios y subredes de Active Directory, para limitar el uso de estos privilegios.
...
- **Privilegios y sistema de protección de dominios:** el bosque administrativo debe estar configurado en el menor nivel de privilegio según los requisitos para la administración de Active Directory.
   ...
   - El bosque administrativo debe respetar las configuraciones de Microsoft Security Compliance Baseline (SCB) para el dominio, incluidas configuraciones seguras para los protocolos de autenticación.
...
- **Servidor y sistema de protección del controlador de dominio**: para todos los controladores de dominio y servidores del bosque administrativo:
   ...
   - Las líneas base de seguridad se deben usar como configuraciones iniciales.
      ...
      > [!NOTE]
      > Los clientes pueden usar Microsoft Security Compliance Toolkit (SCT) para configurar las líneas base en los hosts administrativos.
...
La mayoría de las organizaciones controlan la pertenencia a grupos de Active Directory de nivel 0 eficaces, como administradores, administradores de dominio y administradores de empresa.  Muchas organizaciones pasan por alto el riesgo de otros grupos que son eficazmente equivalentes con privilegios de un entorno típico de Active Directory. Estos grupos ofrecen una ruta de escalamiento relativamente sencilla para un atacante a los mismos privilegios de nivel 0 explícitos mediante el uso de varios métodos de ataque diferentes.
...
- Otros grupos delegados: los grupos personalizados que puede crear tu organización para administrar operaciones de directorio que también pueden tener un acceso efectivo de nivel 0.
...
- **#** es el identificador numérico para el tipo de inicio de sesión que se notifica en los eventos de auditoría en el registro de eventos de seguridad.
