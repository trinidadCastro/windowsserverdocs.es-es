...
| ---    | ---  | ---       |
| ☐    | Comprobar los requisitos previos | [Comprobación de los requisitos previos](#verify-the-prerequisites) |
...
- Asegúrate de que se cumpla una de las siguientes condiciones:
    ...
    - Si el usuario no tiene el permiso **Crear objetos de equipo** , solicita a un administrador de dominio que preconfigure un objeto de equipo de clúster para el clúster. Para obtener más información, consulte [Prestage Cluster Computer Objects in Active Directory Domain Services](prestage-cluster-adds.md).

> [!NOTE]
> Este requisito no se aplica si desea crear un clúster desconectado Active Directory en Windows Server 2012 R2. Para obtener más información, consulte [Deploy an Active Directory-Detached Cluster](https://docs.microsoft.com/previous-versions/windows/it-pro/windows-server-2012-r2-and-2012/dn265970(v=ws.11)).
...
Para completar este paso, asegúrate de que la cuenta de usuario con la que inicias sesión cumple los requisitos indicados en la sección [Comprobar los requisitos previos](#verify-the-prerequisites) de este tema.
...
1. Utiliza el Administrador del servidor o Windows PowerShell para instalar el rol o la característica que sean necesarios para un rol en clúster en cada nodo de clúster de conmutación. Por ejemplo, si deseas crear un servidor de archivos en clúster, instala el rol de Servidor de archivos en todos los nodos de clúster.
   ...
   | Coordinador de transacciones distribuidas (DTC)     | Ninguna        |
   | Servidor de archivos     |  Rol de Servidor de archivos       |
   | Aplicación genérica     |  No corresponde       |
   | Script genérico     |   No corresponde      |
   | Servicio genérico     |   No corresponde      |
