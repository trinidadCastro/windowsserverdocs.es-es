...
Hay una serie de motivos por los que puede que falte un disco o que no se haya podido inicializar, con un motivo común porque el disco está fallando. Hay muchas cosas que se pueden hacer para corregir un error de disco, pero, a continuación, se indican algunos pasos para ver si conseguimos que funcione de nuevo. Si el disco funciona después de realizar uno de estos pasos, no te preocupes por los siguientes; simplemente relájate, celébralo y, a lo mejor, actualiza las copias de seguridad.
...
2. Si el disco aparece en la Administración de discos como *Conectado* y tiene una partición principal que aparece como *Correcto*, como se muestra aquí, es una buena señal.
    ...
    - Si la partición tiene un sistema de archivos, pero ninguna letra de unidad (por ejemplo, E:), consulta [Change a drive letter](change-a-drive-letter.md) (Cambiar una letra de unidad) para agregar una letra de unidad manualmente.
    - Si no tienes un sistema de archivos (NTFS, ReFS, FAT32 o exFAT) y sabes que el disco está vacío, haz clic con el botón derecho en la partición y selecciona **Formatear**. Al formatear un disco, se borran todos los datos que contiene, por lo que no lo hagas si estás intentando recuperar archivos del disco; en su lugar, ve al paso siguiente.
3. Si tienes un disco externo, desconéctalo y vuelve a conectarlo y, a continuación, selecciona **Acción** > **Volver a examinar los discos**. 
...
7. Comprueba el Administrador de dispositivos para ver si hay problemas.
    Mantén presionado el botón Inicio (o haz clic en él con el botón derecho) y, a continuación, selecciona el Administrador de dispositivos en el menú contextual. Busca dispositivos con un signo de exclamación al lado u otros problemas, haz doble clic en el dispositivo y, a continuación, lee su estado.

    Esta es una lista de [Códigos de error en el Administrador de dispositivos](https://support.microsoft.com/help/310123/error-codes-in-device-manager-in-windows), pero un enfoque que a veces funciona consiste en hacer clic con el botón derecho en el dispositivo problemático, seleccionar **Desinstalar dispositivo** y, a continuación, **Acción** > **Buscar cambios de hardware**.
...
4. Prueba también los pasos de solución de problemas descritos en [El estado de un disco aparece como No inicializado o falta el disco completamente](#a-disks-status-is-not-initialized-or-the-disk-is-missing).
...
- Prueba también los pasos de solución de problemas descritos en [El estado de un disco aparece como No inicializado o falta el disco completamente](#a-disks-status-is-not-initialized-or-the-disk-is-missing).
...
2. Haz clic con el botón derecho en el disco y, a continuación, haz clic en **Nuevo volumen**.
