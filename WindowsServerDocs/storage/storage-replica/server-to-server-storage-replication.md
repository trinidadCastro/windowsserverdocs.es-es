...
description: Cómo configurar y usar réplica de almacenamiento para la replicación de servidor a servidor en Windows Server, incluidos Windows Admin Center y PowerShell.
...
Para administrar la réplica de almacenamiento puede usar [Windows Admin Center](../../manage/windows-admin-center/overview.md) o PowerShell.

Este es un vídeo de información general del uso de réplica de almacenamiento en Windows Admin Center.
...
* Una red entre servidores con ancho de banda suficiente para contener la carga de trabajo de escritura de E/S y un promedio de =5 ms de latencia de ida y vuelta para la replicación sincrónica. Replicación asincrónica no tiene una recomendación de latencia.<br>
Si va a replicar entre servidores locales y máquinas virtuales de Azure, debe crear un vínculo de red entre los servidores locales y las máquinas virtuales de Azure. Para ello, utilice [Express Route](#add-azure-vm-expressroute), un [conexión de puerta de enlace VPN de sitio a sitio](https://docs.microsoft.com/azure/vpn-gateway/vpn-gateway-howto-site-to-site-resource-manager-portal), o instalar software de VPN en las máquinas virtuales de Azure para conectarlos con la red local.
...
Para usar réplica de almacenamiento y Windows Admin Center juntos, necesita lo siguiente:
...
> [!NOTE]
> En estos momentos no se puede usar Windows Admin Center en un servidor para administrar la réplica de almacenamiento.
...
**Figura 1: Servidor para replicación de servidor**  

## <a name="step-1-install-and-configure-windows-admin-center-on-your-pc"></a>Paso 1: Instalar y configurar Windows Admin Center en su PC

Si usa Windows Admin Center para administrar la réplica de almacenamiento, siga estos pasos para preparar su PC para administrar la réplica de almacenamiento.
1. Descargue e instale [Windows Admin Center](../../manage/windows-admin-center/overview.md).
2. Descargue e instale el [herramientas de administración remota del servidor](https://www.microsoft.com/download/details.aspx?id=45520).
    - Si usa Windows 10, versión 1809 o posterior, instale el "RSAT: Réplica módulo de almacenamiento para Windows PowerShell"de las características a petición.
3. Abra una sesión de PowerShell como administrador seleccionando el **iniciar** button, escribir **PowerShell**, hacer clic **Windows PowerShell,** y, a continuación, seleccione  **Ejecutar como administrador**.
4. Escriba el siguiente comando para habilitar el protocolo WS-Management en el equipo local y establecer la configuración predeterminada para la administración remota en el cliente.

    ```PowerShell
    winrm quickconfig
    ```

5. Tipo **Y** para habilitar WinRM services y habilitar la excepción de Firewall de WinRM.
...
3.  Agregar información de la red, unir los servidores al mismo dominio que su PC (si está utilizando uno) de administración de Windows 10 y, a continuación, reinicie los servidores.  
...
3.  Conecte el primer conjunto de destino iSCSI, almacenamiento JBOD, SAN de FC o almacenamiento de disco fijo local (DAS) al servidor de sitio **Redmond**.  

4.  Conecte el segundo conjunto de almacenamiento en el servidor de sitio **Bellevue**.  
...
7.  Configure los roles de la manera siguiente:  
    ...
    -   **Método Windows Admin Center**
        1. En Windows Admin Center, vaya al administrador del servidor y, a continuación, seleccione uno de los servidores.
        2. Vaya a **Roles y características**.
        3. Seleccione **características** > **réplica de almacenamiento**y, a continuación, haga clic en **instalar**.
        4. Repita en el otro servidor.
    -   **El administrador del servidor (método)**  
...
8.  Configure el almacenamiento como sigue:  
    ...
    - **Para el almacenamiento de disco fijo local:**  

        -   Asegúrese del almacenamiento no contienen un volumen del sistema, el archivo de paginación o archivos de volcado.  
...
10. Examine el **TestSrTopologyReport.html** informe que se muestra en la figura 2 para asegurarse de que cumplen los requisitos de réplica de almacenamiento.  
...
1. Agregue el servidor de origen.
    1. Seleccione el **agregar** botón.
    2. Seleccione **Agregar conexión de servidor**.
    3. Escriba el nombre del servidor y, a continuación, seleccione **enviar**.
2. En el **todas las conexiones** , seleccione el servidor de origen.
...
> [!NOTE]
> Quitar la asociación de réplica de almacenamiento en Windows Admin Center no quita el nombre del grupo de replicación.
...
* No se replican archivos abiertos o en uso.  
* No se replican sincrónicamente.  
...
* Por lo general se configura como varios maestros, que permite cambios fluyan en ambas direcciones, sobrescribiendo posiblemente los datos más recientes.  
...
* Aunque admite la replicación asincrónica, no está diseñado para el ancho de banda bajo, redes de latencia alta.  
* No permite acceso de usuario a los datos protegidos en el destino mientras la replicación está en curso  
...
1. [Creación de ExpressRoute en el portal de Azure](https://docs.microsoft.com/azure/expressroute/expressroute-howto-circuit-portal-resource-manager).<br>Cuando se aprueba el circuito, un grupo de recursos se agrega a la suscripción -, navegue hasta **grupos de recursos** para ver este grupo nuevo. Tome nota del nombre de red virtual.
![Azure portal que muestra el grupo de recursos que se agregan con ExpressRoute](media/Server-to-Server-Storage-Replication/express-route-resource-group.png)
    
    **Figura 4: Los recursos asociados de ExpressRoute: tome nota del nombre de red virtual**
1. [Crear un nuevo grupo de recursos](https://docs.microsoft.com/azure/azure-resource-manager/resource-group-portal).
1. [Agregar un grupo de seguridad de red](https://docs.microsoft.com/azure/virtual-network/virtual-networks-create-nsg-arm-pportal). Cuando se crea, seleccione el identificador de suscripción asociado con el circuito que creó y seleccione el grupo de recursos que acaba de crear también.
<br><br>Agregue las reglas de seguridad entrante y saliente que necesita para el grupo de seguridad de red. Por ejemplo, es posible que desee permitir el acceso de escritorio remoto a la máquina virtual.
