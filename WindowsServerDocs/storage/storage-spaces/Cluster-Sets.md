...
2. Una vez que se hayan creado todos los clústeres, use el siguiente comando para crear el clúster de configuración maestra:

    ```PowerShell
    New-ClusterSet -Name CSMASTER -NamespaceRoot SOFS-CLUSTERSET -CimSession SET-CLUSTER
    ```

3. Use el comando siguiente para agregar un servidor de clúster al conjunto de clústeres:
...
8. Para comprobar que el proceso de creación del conjunto de clústeres ha creado un recurso compartido de SMB (identificado como volume1, o con independencia de la etiqueta de la carpeta CSV, el nombre del servidor de archivos de infraestructura y la ruta de acceso son ambos) en el SOFS de infraestructura para el volumen CSV de cada miembro del clúster:

    ```PowerShell
    Get-SmbShare -CimSession CSMASTER
    ```

9. Los registros de depuración del conjunto de clústeres se pueden recopilar para su revisión. Tanto el conjunto de clústeres como los registros de depuración de clúster se pueden recopilar para todos los miembros y el clúster de administración:

    ```PowerShell
    Get-ClusterSetLog -ClusterSetCimSession CSMASTER -IncludeClusterLog -IncludeManagementClusterLog -DestinationFolderPath <path>
    ```

10. Configure la [delegación restringida](https://techcommunity.microsoft.com/t5/virtualization/live-migration-via-constrained-delegation-with-kerberos-in/ba-p/382334) de Kerberos entre todos los miembros del conjunto de clústeres.

11. Configure el tipo de autenticación de migración en vivo de máquinas virtuales entre clústeres en Kerberos en cada nodo del conjunto de clústeres.

    ```PowerShell
    foreach($h in $hosts){ Set-VMHost -VirtualMachineMigrationAuthenticationType Kerberos -ComputerName $h }
    ```

12. Agregue el clúster de administración al grupo de administradores locales en cada nodo del conjunto de clústeres.
...
Si no tiene suficiente memoria, capacidad de CPU o espacio en disco para agregar la máquina virtual, recibirá el siguiente error:

```
Get-ClusterSetOptimalNodeForVM : A cluster node is not available for this operation.
```

Una vez creada la máquina virtual, se mostrará en el administrador de Hyper-V en el nodo específico especificado. Para agregarlo como una máquina virtual de conjunto de clústeres y agregarlo al clúster, use el siguiente comando:
...
Si ha agregado un clúster con máquinas virtuales existentes, las máquinas virtuales también tendrán que registrarse con conjuntos de clústeres. Para registrar todas esas máquinas virtuales a la vez, el comando que se va a usar es:

```PowerShell
Get-ClusterSetMember -Name CLUSTER3 -CimSession CSMASTER | Register-ClusterSetVM -RegisterAll -CimSession CSMASTER
```

Sin embargo, aún no se ha completado el proceso, ya que la ruta de acceso a la máquina virtual debe agregarse al espacio de nombres del conjunto de clústeres.
