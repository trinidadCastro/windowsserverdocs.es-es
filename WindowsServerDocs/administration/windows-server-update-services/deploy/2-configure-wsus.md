...
description: 'Tema de Windows Server Update Service (WSUS): configurar WSUS es el paso 2 en un proceso de cuatro pasos para la implementación de WSUS'
...
ms.locfileid: "59820486"
---
# <a name="step-2-configure-wsus"></a>Paso 2: Configurar WSUS

>Se aplica a: Windows Server (canal semianual), Windows Server 2016, Windows Server 2012 R2, Windows Server 2012

Después de instalar el rol de servidor de WSUS en su servidor, debe configurarlo de forma apropiada. La siguiente lista de comprobación resume los pasos necesarios para llevar a cabo la configuración inicial para el servidor WSUS.

|Tarea|Descripción|
|----|--------|
|[2.1. Configurar conexiones de red](2-configure-wsus.md#BKM_ConfigurenetworkConnections)|Configure la red en clúster con el Asistente para configuración de red.|
|[2.2. Configurar WSUS mediante el Asistente para configuración de WSUS](2-configure-wsus.md#BKM_ConfigureWSUSusingConfigurationWizard)|Utilizar el Asistente para configuración de WSUS a fin de realizar la configuración básica de WSUS.|
|[2.3. Configurar grupos de equipos WSUS](2-configure-wsus.md#BKMK_ConfigcomputerGroups)|Cree grupos de equipos en la consola de administración de WSUS para administrar las actualizaciones de su organización.|
|[2.4. Configurar actualizaciones de cliente](2-configure-wsus.md#BKM_ConfigureClientUpdates)|Especifique cómo y cuándo se aplican las actualizaciones automáticas en los equipos cliente.|
|[2.5. Proteger WSUS con el protocolo de capa de Sockets seguros](2-configure-wsus.md#bkmk_2.5.ConfigSSL)|Configure el protocolo de Capa de sockets seguros (SSL) para ayudar a proteger Windows Server Update Services (WSUS).|

## <a name="BKM_ConfigurenetworkConnections"></a>2.1. Configurar conexiones de red
...
De manera predeterminada, WSUS se configura para usar Microsoft Update como ubicación donde obtener actualizaciones. Si tiene un servidor proxy en la red, puede configurar WSUS para usar el servidor proxy. Si hay un firewall corporativo entre WSUS e Internet, tendrá que configurar el firewall para asegurarse de que WSUS obtenga actualizaciones.
...
-   **Proxy** si observa que WSUS necesita utilizar un servidor proxy para tener acceso a Internet, deberá configurar el proxy en el servidor WSUS.

-   **Firewall** si observa que WSUS esté protegido por un firewall corporativo, hay algunos pasos adicionales que deben realizarse en el dispositivo perimetral para permitir el tráfico de WSUS correctamente.

### <a name="211-connection-from-the-wsus-server-to-the-internet"></a>2.1.1. Conexión desde el servidor WSUS a Internet
Si hay un firewall corporativo entre WSUS e Internet, tendrá que configurar el firewall para asegurarse de que WSUS obtenga actualizaciones. Para obtener actualizaciones de Microsoft Update, el servidor WSUS usa el puerto 443 para el protocolo HTTPS. Aunque la mayoría de los firewalls corporativos permite a este tipo de tráfico, hay algunas compañías restringen el acceso a Internet desde los servidores debido a las directivas de seguridad de la empresa. Si su compañía restringe el acceso, deberá obtener autorización para permitir el acceso a Internet desde WSUS a la lista de direcciones URL siguiente:
...
> [!IMPORTANT]
> Para un escenario en el que WSUS no puede obtener actualizaciones debido a la configuración de firewall, consulte [artículo 885819](https://support.microsoft.com/kb/885819) en Microsoft Knowledge Base.
...
-   En WSUS 6.2 y versiones posteriores (al menos Windows Server 2012), el puerto 8530 para HTTP y 8531 para HTTPS se usan
...
3.  Abra un símbolo del sistema (Cmd.exe) como administrador. Para abrir un símbolo del sistema como administrador, vaya a **Inicio**. En **Iniciar búsqueda**, tipo **símbolo**. en la parte superior del menú Inicio, haga clic en **símbolo**y, a continuación, haga clic en **ejecutar como administrador**. Si el **User Account Control** aparece el cuadro de diálogo, escriba las credenciales adecuadas (si se solicitan), confirme que la acción que se muestra es lo que desea y, a continuación, haga clic en **continuar**.

4.  En la ventana de símbolo del sistema, vaya a la carpeta C:\Program programa\Update Services\Tools. Escriba el siguiente comando:

    **WSUSUTIL ConfigureSSlproxy [< proxy_server proxy_port >]-habilitar**, donde:

    1.  proxy_server es el nombre del servidor proxy que admite HTTPS.

    2.  proxy_port es el número del puerto del servidor proxy.

5.  Cierre la ventana de símbolo del sistema.
...
4.  Use las siguientes opciones para modificar la configuración del servidor proxy existente:
    ...
    4.  Desactivar el servidor proxy requiere el uso de una cuenta de usuario, seleccione el **usar las credenciales de usuario para conectarse al servidor proxy** casilla de verificación. Escriba el nombre de usuario requerido, el dominio y la contraseña en los cuadros de texto correspondientes.
...
## <a name="BKM_ConfigureWSUSusingConfigurationWizard"></a>2.2. Configurar WSUS con el Asistente para la configuración de WSUS
...
1.  En el panel de navegación del Administrador del servidor, haga clic en **Panel**, en **Herramientas** y, a continuación, en **Windows Server Update Services**.
    ...
    > [!NOTE]
    > Si el **completar la instalación de WSUS** aparece el cuadro de diálogo, haga clic en **ejecutar**. En el **completar la instalación de WSUS** cuadro de diálogo, haga clic en **cerrar** cuando la instalación finalice correctamente.
...
3.  Lea las instrucciones en la página **Unirse al programa de mejora de Microsoft Update** y determine si desea participar. Si quiere participar en el programa. Conserve la selección predeterminada, o desactive la casilla de verificación y, a continuación, haga clic en **siguiente**.
...
4.  En la página **Elegir servidor que precede en la cadena** , hay dos opciones:
    ...
    2.  Sincronizar desde otro servidor de Windows Server Update Services.
        ...
        -   Si decide sincronizar desde otro servidor WSUS, especifique el nombre del servidor y el puerto en el que este servidor se comunicará con el servidor que precede.
        ...
        -   Si se trata de un servidor de réplica, seleccione el **se trata de una réplica del servidor ascendente** casilla de verificación.
...
7.  Si desea conectarse al servidor proxy con credenciales de usuario específicas, seleccione el **usar las credenciales de usuario para conectarse al servidor proxy** casilla de verificación y, a continuación, escriba el nombre de usuario, dominio y contraseña del usuario en las correspondientes cuadros. Si desea habilitar la autenticación básica para el usuario que se conecta al servidor proxy, seleccione el **permitir autenticación básica (la contraseña se envía en texto no cifrado)** casilla de verificación.

8.  Haz clic en **Siguiente**. En el **conectar al servidor que precede en** página, haga clic en **iniciar conexión**.

9. Cuando se conecte, haga clic en **Siguiente** para continuar.

10. En el **Elegir idiomas** página, tiene la opción para seleccionar los idiomas en el que WSUS recibirá actualizaciones - todos los idiomas o un subconjunto de idiomas. Seleccionar un subconjunto de idiomas, ahorrará espacio en disco, pero es importante elegir todos los idiomas que son necesarios para todos los clientes de este servidor WSUS. Si elige obtener actualizaciones solo de idiomas concretos, seleccione **descargar actualizaciones solamente en estos idiomas**y, a continuación, seleccione los idiomas para los que desea; de lo contrario, deje la selección predeterminada.

    > [!WARNING]
    > Si selecciona la opción **descargar actualizaciones solamente en estos idiomas**y este servidor tiene un servidor WSUS que sigue conectado a ella, esta opción forzará al servidor de nivel inferior para usar solo los idiomas seleccionados.

11. Una vez que seleccione las opciones de idioma apropiadas para la implementación, haga clic en **Siguiente** para continuar.

12. La página **Elegir productos** le permite especificar los productos para los que desea actualizaciones. Seleccione las categorías de producto, como Windows o productos específicos, como Windows Server 2008. al seleccionar una categoría de producto selecciona todos los productos de esa categoría.

13. Seleccione las opciones de producto apropiadas para su implementación y, a continuación, haga clic en **siguiente**.
...
15. En la página **Establecer una programación de sincronización** se permite seleccionar si se debe realizar la sincronización de forma manual o automática.
    ...
    -   Si elige **sincronizar automáticamente**, el servidor WSUS sincronizará en intervalos establecidos.
...
1.  En la consola de administración de WSUS en **Update Services**, expanda el servidor WSUS, expanda **equipos**, haga clic en **todos los equipos**y, a continuación, haga clic en **Agregar equipo de grupo**.

2.  En el **Agregar grupo de equipo** cuadro de diálogo **nombre**, especifique el nombre del nuevo grupo y, a continuación, en **agregar**.

3.  Haga clic en **equipos**y, a continuación, seleccione los equipos que desea asignar a este grupo nuevo.

4.  Haga clic en los nombres de equipo que seleccionó en el paso anterior y, a continuación, haga clic en **cambiar pertenencia**.

5.  En el **establecer pertenencia a grupos del equipo** cuadro de diálogo, seleccione la prueba de grupo que creó y, a continuación, haga clic en **Aceptar**.

## <a name="BKM_ConfigureClientUpdates"></a>2.4. Configurar actualizaciones de cliente
...
-   En un entorno que usa el servicio de directorio active directory, puede usar una existente basado en dominio directiva objeto grupo (GPO) o crear un nuevo GPO.

-   En un entorno sin active directory, utilice el editor de directivas de grupo Local para configurar actualizaciones automáticas y, a continuación, seleccione los equipos cliente al servidor WSUS.

> [!IMPORTANT]
> Los procedimientos siguientes suponen que su red ejecuta active directory. Además se supone que usted está familiarizado con la directiva de grupo y que la usa para administrar la red.
...
-   [2.3. Configurar grupos de equipos](2-configure-wsus.md#BKMK_ConfigcomputerGroups) en este tema.

### <a name="BKMK_ConfigureAU"></a>Configurar actualizaciones automáticas en la directiva de grupo
Si ha configurado active directory en la red, puede configurar uno o varios equipos simultáneamente al incluirlos en un objeto de directiva de grupo (GPO) y, a continuación, configurar ese GPO con la configuración de WSUS. Se recomienda que cree un GPO nuevo, que contenga solo la configuración de WSUS.
...
1.  En la consola de administración de directivas de grupo (GPMC), busque el GPO en el que desea configurar WSUS y, a continuación, haga clic en **editar**.

2.  En GPMC, expanda **equipo configuración**, expanda **directivas**, expanda **plantillas administrativas**, expanda **componentes de Windows**y, a continuación, haga clic en **Windows Update**.
...
9. Haga clic en **Habilitado** y escriba la misma dirección URL del servidor WSUS en los cuadros de texto **Establecer el servicio de actualización de la intranet para detectar actualizaciones** y **Establecer el servidor de estadísticas de la intranet**. Por ejemplo, escriba *http://servername* en ambos cuadros (donde *servername* es el nombre del servidor WSUS).
    ...
    > [!WARNING]
    > Cuando escriba la dirección de intranet de su servidor WSUS, asegúrese de especificar el puerto que se va a usar. De manera predeterminada, WSUS usará el puerto 8530 para HTTP y 8531 para HTTPS. Por ejemplo, si está utilizando HTTP, debe escribir **http://servername:8530**.

10. Haga clic en **Aceptar**.

Después de configurar un equipo cliente, se tardará varios minutos antes de que el equipo aparece en la **equipos** página en la consola de administración de WSUS. Con equipos cliente configurados con un objeto de directiva de grupo basado en dominio, pueden transcurrir alrededor de 20 minutos para que la directiva de grupo se aplique a la nueva configuración del equipo cliente. De forma predeterminada, las actualizaciones de directiva de grupo en segundo plano cada 90 minutos, con un desplazamiento aleatorio de 0 a 30 minutos. Si desea actualizar la directiva de grupo más pronto, puede abrir una ventana de símbolo del sistema en el cliente y el tipo gpupdate /force.

para los equipos cliente que se configuran mediante el editor de directivas de grupo Local, el GPO se aplica inmediatamente y la actualización tarda aproximadamente 20 minutos. Si comienza la detección manualmente, no es necesario que espere 20 minutos para el equipo cliente se comunique con WSUS.
...
1.  En el equipo cliente, abra una ventana de símbolo del sistema con privilegios elevados.

2.  escriba wuauclt.exe /detectnow y, a continuación, presione ENTRAR.

## <a name="bkmk_2.5.ConfigSSL"></a>2.5. Proteger WSUS con el protocolo de Capa de sockets seguros
...
2.  Si usa WSUS con una base de datos de SQL Server remoto, la conexión entre el servidor WSUS y el servidor de base de datos no está protegida por SSL. Si la conexión de base de datos debe estar protegida, tenga en cuenta las siguientes recomendaciones:

-   mover la base de datos WSUS en el servidor WSUS.

-   mover el servidor de base de datos remota y el servidor WSUS a una red privada.

-   Implemente el protocolo de seguridad de Internet (IPsec) para ayudar a proteger el tráfico de la red. Para más información sobre IPsec, consulte [Creación y uso de directivas de IPsec](https://go.microsoft.com/fwlink/?LinkID=203841).

### <a name="consswsus"></a>Configurar SSL en el servidor WSUS
...
-   El certificado de la entidad de certificación (CA) debe importarse en el almacén de CA raíz de confianza del equipo local o el de Windows Server Update Service en los servidores WSUS que siguen en la cadena. Si solo se importa el certificado a la CA raíz de confianza de usuario Local almacén, no se autenticará en el servidor que precede en el servidor WSUS que sigue.

    Para obtener más información sobre cómo usar certificados SSL en IIS, consulte [requieren Secure Sockets Layer (IIS 7)](https://go.microsoft.com/fwlink/?LinkID=203846).
...
-   Puede usar cualquier puerto para SSL. Sin embargo, el puerto que configure para SSL también determina el puerto que WSUS usa para enviar el tráfico HTTP sin cifrar. Revisa los siguientes ejemplos:
    ...
    -   Si usa el puerto estándar del sector 443 para el tráfico HTTPS, WSUS usa el puerto estándar del sector 80 para tráfico HTTP sin cifrar.

    -   Si usa un puerto distinto del 443 para el tráfico HTTPS, WSUS enviará tráfico HTTP sin cifrar a través del puerto que preceda numéricamente el puerto para HTTPS. Por ejemplo, si utiliza el puerto 8531 para HTTPS, WSUS usará el puerto 8530 para HTTP.
...
### <a name="consslclient"></a>Configurar SSL en los equipos cliente
Al configurar SSL en los equipos cliente, debe considerar lo siguiente:

-   Debe incluir la dirección URL de un puerto seguro en el servidor WSUS. Dado que no se requiere SSL en el servidor, la única forma de asegurarse de que los equipos cliente pueden usar un canal de seguridad es mediante una dirección URL que especifique HTTPS. Si usa un puerto distinto del 443 para SSL, también debe incluir ese puerto en la dirección URL.

-   El certificado en un equipo cliente debe importarse en el almacén de CA raíz de confianza del equipo Local o el almacén de CA raíz de confianza del servicio de actualización automática. Si el certificado se importa al almacén de CA raíz de confianza del usuario Local, las actualizaciones automáticas se producirá un error de autenticación de servidor.
...
### <a name="consslds"></a>Configurar SSL para servidores WSUS que siguen
...
2.  Haga clic en **iniciar**, haga clic en **todos los programas**, haga clic en **herramientas administrativas**y, a continuación, haga clic en **Windows Server Update Services**.
...
6.  Escriba el nombre del servidor de nivel superior en el **nombre del servidor** cuadro de texto. Escriba el número de puerto que utiliza el servidor para las conexiones SSL en el **número de puerto** cuadro de texto.

7.  Seleccione el **usar SSL al sincronizar la información de actualización** casilla de verificación y, a continuación, haga clic en **Aceptar**.

### <a name="addlsl"></a>recursos SSL adicionales
...
-   [Guía paso a paso de PKI Suite B](https://go.microsoft.com/fwlink/?LinkID=203858)

-   [Implementación y administración de plantillas de certificado](https://go.microsoft.com/fwlink/?LinkID=203859)

-   [Active directory Certificate Services actualice y Guía de migración](https://go.microsoft.com/fwlink/?LinkID=203860)
...
3.  **Actualizaciones y revocación de certificados**. Si desea actualizar o revocar un certificado (es decir, después de detectar que expiró), WSUS no ofrecía ninguna funcionalidad para habilitar esta opción. Para realizar esto, era necesario realizar una tarea manual que era muy complicada de hacer a mano o automatizar correctamente.
